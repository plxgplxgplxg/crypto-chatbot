      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div className="border-b border-[var(--border-color)]">
          <div className="max-w-3xl mx-auto px-3 sm:px-4 py-3 sm:py-4 flex items-center justify-between">
            <h1 className="text-lg sm:text-xl font-semibold text-[var(--text-primary)]">Chat với AI Crypto</h1>
            {messages.length > 0 && (
              <button
                onClick={clearHistory}
                className="text-sm sm:text-base text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors"
              >
                Xóa lịch sử
              </button>
            )}
          ...
          <div className="max-w-3xl mx-auto px-3 sm:px-4 py-4 sm:py-6">
            {messages.length === 0 && !loading && (
              <div className="text-center text-[var(--text-secondary)] py-12 sm:py-20">
                <p className="text-lg sm:text-xl mb-2 sm:mb-3">Chào mừng bạn đến với Crypto AI</p>
                <p className="text-sm sm:text-base">Hãy hỏi tôi về phân tích thị trường crypto.</p>
              </div>
            )}
